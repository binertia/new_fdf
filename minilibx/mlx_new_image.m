...
void *mlx_new_image(mlx_ptr_t *mlx_ptr, int width, int height)
{
	...
	//    return (NULL);  // need at leat one window created to have openGL
	//    context and create texture
	...
	newimg->vertexes[0] = 0.0;
	newimg->vertexes[1] = 0.0;
	newimg->vertexes[2] = width;
	newimg->vertexes[3] = 0.0;
	newimg->vertexes[4] = width;
	newimg->vertexes[5] = -height;
	newimg->vertexes[6] = 0.0;
	newimg->vertexes[7] = -height;
...
mlx_img_ctx_t *add_img_to_ctx(mlx_img_list_t *img, mlx_win_list_t *win)
{
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(img->vertexes), img->vertexes,
				 GL_DYNAMIC_DRAW); // 4 points buff

	return (imgctx);
}

void mlx_put_image_to_window(mlx_ptr_t *mlx_ptr, mlx_win_list_t *win_ptr,
...
char *mlx_get_data_addr(mlx_img_list_t *img_ptr, int *bits_per_pixel,
...
int mlx_string_put(mlx_ptr_t *mlx_ptr, mlx_win_list_t *win_ptr, int x, int y,
				   int color, unsigned char *string)
{
	mlx_img_ctx_t *imgctx;
	int gX;
	int gY;
